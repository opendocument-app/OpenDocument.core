...
typedef std::function<void(const CompoundFileEntry *, const std::u16string &dir, int level)> EnumFilesCallback;
...
public:
    CompoundFileReader(const void *buffer, const std::size_t len) :
        ...
        if (buffer == nullptr || len == 0) throw std::invalid_argument("");
        ...
        if (m_bufferLen < m_sectorSize * 3) throw CfbFileCorruptedException();

        const CompoundFileEntry *root = GetEntry(0);
        if (root == nullptr) throw CfbFileCorruptedException();
    ...
    const CompoundFileEntry *GetEntry(const std::size_t entryID) const {
        ...
        std::size_t sector = 0;
        std::size_t offset = 0;
        LocateFinalSector(m_hdr->firstDirectorySectorLocation, entryID * sizeof(CompoundFileEntry), &sector, &offset);
        return reinterpret_cast<const CompoundFileEntry *>(SectorOffsetToAddress(sector, offset));
    }

    const CompoundFileEntry *GetRootEntry() const {
        return GetEntry(0);
    }

    const CompoundFileHeader *GetFileInfo() const {
        return m_hdr;
    }
    ...
    void ReadFile(const CompoundFileEntry *entry, const std::size_t offset, char *buffer, const std::size_t len) const {
    ...
    void EnumFiles(const CompoundFileEntry *entry, int maxLevel, EnumFilesCallback callback) const {
        std::u16string dir;
...
private:
    ...
    void EnumNodes(const CompoundFileEntry *entry, const int currentLevel, const int maxLevel,
        ...
        if (child != nullptr) {
            ...
            if (dir.length() != 0) newDir.append(1, '\n');
    ...
    void ReadStream(std::size_t sector, std::size_t offset, char *buffer, std::size_t len) const {
        ...
        while (len > 0) {
            ...
            if (m_buffer + m_bufferLen < src + copylen) throw CfbFileCorruptedException();
    ...
    void ReadMiniStream(std::size_t sector, std::size_t offset, char *buffer, std::size_t len) const {
    ...
    std::size_t GetNextSector(std::size_t sector) const {
        ...
        return ParseUint32(SectorOffsetToAddress(fatSectorLocation, sector % entriesPerSector * 4));
    }
    ...
    const std::uint8_t *SectorOffsetToAddress(std::size_t sector, std::size_t offset) const {
        if (sector >= MAX_REG_SECT ||
                offset >= m_sectorSize ||
                m_bufferLen <= static_cast<std::uint64_t>(m_sectorSize) * sector + m_sectorSize + offset) {
    ...
    const std::uint8_t *MiniSectorOffsetToAddress(std::size_t sector, std::size_t offset) const {
        if (sector >= MAX_REG_SECT ||
            offset >= m_miniSectorSize ||
                m_bufferLen <= static_cast<std::uint64_t>(m_miniSectorSize) * sector + offset) {
            throw CfbFileCorruptedException();
        }
    ...
    void LocateFinalSector(std::size_t sector, std::size_t offset, std::size_t *finalSector, std::size_t *finalOffset) const {
    ...
    void LocateFinalMiniSector(std::size_t sector, std::size_t offset, std::size_t *finalSector, std::size_t *finalOffset) const {
    ...
    std::size_t GetFATSectorLocation(std::size_t fatSectorNumber) const {
        if (fatSectorNumber < 109) {
            ...
            while (fatSectorNumber >= entriesPerSector) {
                ...
                const std::uint8_t *addr = SectorOffsetToAddress(difatSectorLocation, m_sectorSize - 4);
                difatSectorLocation = ParseUint32(addr);
            }
            return ParseUint32(SectorOffsetToAddress(difatSectorLocation, fatSectorNumber * 4));
        }
    }
    ...
    const std::uint16_t *GetStringProperty(std::uint32_t propertyID) {
        for (std::uint32_t i = 0; i < m_hdr->numProperties; i++) {
            if (m_hdr->propertyIdentifierAndOffset[i].id == propertyID) {
                ...
                if (m_bufferLen < offset + 8) throw CfbFileCorruptedException();
                std::uint32_t stringLengthInChar = ParseUint32(m_buffer + offset + 4);
                if (m_bufferLen < offset + 8 + stringLengthInChar*2) throw CfbFileCorruptedException();
...
public:
    ...
    std::size_t GetPropertySetCount() {
        return m_hdr->numPropertySets;
    }

    PropertySet GetPropertySet(std::size_t index) {
        if (index >= GetPropertySetCount()) throw CfbFileCorruptedException();
        std::uint32_t offset = m_hdr->propertySetInfo[index].offset;
        if (m_bufferLen < offset + 4) throw CfbFileCorruptedException();
        std::uint32_t size = ParseUint32(m_buffer + offset);
        if (m_bufferLen < offset + size) throw CfbFileCorruptedException();
...
typedef std::function<void(const CFB::CompoundFileEntry *, const Path &)> CfbVisitor;
...
public:
    ...
    public:
        ...
        explicit SourceImpl(const CFB::CompoundFileReader &reader, const CFB::CompoundFileEntry &entry) :
                reader(reader),
                entry(&entry) {
            offset = 0;
        }
        ...
        std::uint32_t available() const final {
            return entry->size - offset;
        }
    };

    explicit Impl(const Path &path) {
        buffer = FileUtil::read(path);
        reader = std::make_unique<CFB::CompoundFileReader>(buffer.data(), buffer.size());
    }

    void visit(CfbVisitor visitor) const {
        reader->EnumFiles(reader->GetRootEntry(), -1, [&](const CFB::CompoundFileEntry *entry, const std::u16string &directory, const int level) {
    ...
    const CFB::CompoundFileEntry *find(const Path &p) const {
        ...
        visit([&](const CFB::CompoundFileEntry *entry, const Path &path) {
            if (p == path) result = entry;
        });
        return result;
    }

    bool isSomething(const Path &p) const {
    ...
    bool isReadable(const Path &p) const {
        return isFile(p);
    }

    std::uint64_t size(const Path &p) const {
        const auto entry = find(p);
        if (entry == nullptr) return 0; // TODO throw?
        return entry->size;
    }
...
CfbReader::CfbReader(const Path &path) :
...
bool CfbReader::isFile(const Path &path) const {
