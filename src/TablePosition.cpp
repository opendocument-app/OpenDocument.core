...
std::uint32_t TablePosition::to_col_num(const std::string &s) {
    ...
    for (std::size_t i = 0; i < s.size(); ++i) {
        if ((s[i] < 'A') || (s[i] > 'Z')) throw std::invalid_argument("illegal character in \"" + s + "\"");
...
TablePosition::TablePosition(const std::uint32_t row, const std::uint32_t col) noexcept :
        row(row), col(col) {
}

TablePosition::TablePosition(const std::string &s) {
    const auto pos = s.find_first_of("0123456789");
    if (pos == std::string::npos) throw std::invalid_argument("malformed table position " + s);
    row = std::stoul(s.substr(pos));
    if (row <= 0) throw std::invalid_argument("row number needs to be at least 1 " + s.substr(pos));
    --row;
