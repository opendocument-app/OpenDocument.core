...
public:
    explicit MsUnsupportedCryptoAlgorithmException(std::string name) : name(std::move(name)) {}
...
public:
    ...
    virtual std::string decrypt(const std::string &encryptedPackage, const std::string &key) const noexcept = 0;
};

class ECMA376Standard final : public Algorithm {
public:
    ECMA376Standard(const EncryptionHeader &, const EncryptionVerifier &, std::string encryptedVerifierHash);
    ...
    std::string decrypt(const std::string &encryptedPackage, const std::string &key) const noexcept final;
