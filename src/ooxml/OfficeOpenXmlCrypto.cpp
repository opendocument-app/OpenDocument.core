...
void toLittleEndian(I in, O &out) {
...
ECMA376Standard::ECMA376Standard(const EncryptionHeader &encryptionHeader, const EncryptionVerifier &encryptionVerifier, std::string encryptedVerifierHash) :
        encryptionHeader(encryptionHeader), encryptionVerifier(encryptionVerifier), encryptedVerifierHash(std::move(encryptedVerifierHash)) {
}
...
ECMA376Standard::ECMA376Standard(const std::string &encryptionInfo) {
    ...
    const std::string CSPName = std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>().to_bytes(CSPNameU16);
    ...
    encryptedVerifierHash = std::string(offset, encryptionInfo.size() - (offset - encryptionInfo.data()));
}

std::string ECMA376Standard::deriveKey(const std::string &password) const noexcept {
    // https://msdn.microsoft.com/en-us/library/dd925430(v=office.12).aspx

    std::string hash;
    {
        const std::u16string passwordU16 = std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>().from_bytes(password);
        const std::string passwordU16Bytes((char *) passwordU16.data(), 2 * passwordU16.size());

        hash = CryptoUtil::sha1(std::string(encryptionVerifier.salt, encryptionVerifier.saltSize) + passwordU16Bytes);
        std::string ibytes(4, ' ');
...
bool ECMA376Standard::verify(const std::string &key) const noexcept {
    ...
    const std::string verifier = CryptoUtil::decryptAES(key, std::string(encryptionVerifier.encryptedVerifier, sizeof(encryptionVerifier.encryptedVerifier)));
    const std::string hash = CryptoUtil::sha1(verifier);
    const std::string verifierHash = CryptoUtil::decryptAES(key, encryptedVerifierHash).substr(0, hash.size());

    return hash == verifierHash;
}

std::string ECMA376Standard::decrypt(const std::string &encryptedPackage, const std::string &key) const noexcept {
    const std::size_t totalSize = *((const std::size_t *) encryptedPackage.data());
    const std::string result = CryptoUtil::decryptAES(key, encryptedPackage.substr(8)).substr(0, totalSize);
...
Util::Util(const std::string &encryptionInfo) {
    ...
    if (((versionInfo.major == 2) || (versionInfo.major == 3) || (versionInfo.major == 4)) && (versionInfo.minor == 2)) {
    ...
    } else if (((versionInfo.major == 3) || (versionInfo.major == 4)) && (versionInfo.minor == 3)) {
...
std::string Util::decrypt(const std::string &encryptedPackage, const std::string &key) const noexcept {
