...
| WP4 | Encryption (standard security handler) — **done** (2026-06) | WP1, WP2 (Encrypt dict can live anywhere; key needs trailer `ID`) |
...
**Done** (2026-06). Implemented as designed in `pdf_encryption.{hpp,cpp}`: a
`Decryptor` parses `/Encrypt`, authenticates user-then-owner passwords, and
decrypts strings/streams. RC4 (V 1/2, R 2/3), AES-128 crypt filters (V 4, R 4;
`StdCF`/`StmF`/`StrF`, `V2`/`AESV2`/`Identity`) and AES-256 (V 5, R 6, AESV3)
are all supported. The pure key/password algorithms are exposed in a
`standard_security` namespace for inline known-answer tests. `crypto::util`
gained `md5`/`rc4`/`sha384`/`sha512` (RC4 and MD5 via Crypto++'s `Weak::`
namespace) and a no-padding `encrypt_aes_cbc` (for the R 6 hardened hash); the
existing `decrypt_aes_cbc` already did raw block processing, so the `UE`/`OE`
unwrap reuses it. `DocumentParser` owns the `Decryptor`: it reads `/Encrypt`
before the decryptor exists (so its strings stay raw), decrypts string leaves
of freshly read used objects and stream bytes before `/Filter` decoding, and
leaves cross-reference streams and object-stream members untouched. `PdfFile`
probes encryption at construction with the empty password and threads the
unlocking password to the HTML service via `abstract::PdfFile::password()`.

Deviations / not done: per-stream `/Crypt` filter `Name` overrides, the
`EncryptMetadata false` metadata-stream `Identity` special case, and `Perms`
(Algorithm 13) validation are left for when a real file needs them; R 5 stays
out of scope. A pre-existing `read_stream` bug (assuming a mandatory EOL before
`endstream`) surfaced via the AES-256 fixture and was fixed alongside.
