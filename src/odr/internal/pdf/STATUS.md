...
graphics, no images, no font files, no encryption (see
[Out of scope](#out-of-scope--open-work)). The module is experimental and not
...
| `pdf_file.{hpp,cpp}`                   | `abstract::PdfFile` wrapper (type tag only; no parsing, no encryption) |
...
- `test/src/internal/pdf/pdf_document_parser.cpp` — **assertion-based**
  ...
  decoding. The `odr-private` xref-stream/objstm/hybrid
  fixtures (`basic_text.pdf`, `geneve_1564.pdf`, `test_fail.pdf`,
  `Kayla….pdf`, `svg_background…issue402.pdf`, `Core_v5.1.pdf`,
  `onepage.pdf`) were verified manually when WP2 landed but are not pinned
  in unit tests — the submodule is not always available; the generic
  all-test-files jobs cover them. Also still contains the original
...
- **Stream filters**: framework done (stage 0 WP1, see above); remaining
  gaps are the image codecs (decoded in stage 4, currently passed through
  encoded) and the non-`Identity` `Crypt` filter (stage 0 WP4, encryption).
- **Encryption**: `password_encrypted()` is hard-coded `false`, `decrypt`
  returns `nullptr`; encrypted files fail somewhere in parsing.
