...
Component::Component(std::string locator, std::string data)
    : m_locator{std::move(locator)},
      m_data{std::make_unique<const std::string>(std::move(data))},
      m_objects{read_objects(*m_data)} {}

const std::string &Component::locator() const noexcept { return m_locator; }

const std::vector<Object> &Component::objects() const noexcept {
  return m_objects;
}

Package::Package(const abstract::ReadableFilesystem &filesystem)
...
const Component &Package::load_(const ComponentInfo &info) {
