...
TEST(OldMs, ppt_parse_style_text_prop_atom) {
  ...
  const auto u16 = [](std::string &out, const std::uint16_t value) {
    out.push_back(static_cast<char>(value & 0xFF));
    out.push_back(static_cast<char>(value >> 8));
  };
  const auto u32 = [&u16](std::string &out, const std::uint32_t value) {
  ...
  u32(body, 12);
  u16(body, 0);          // indentLevel
  u32(body, 0x00000001); // PFMasks: hasBullet
  u16(body, 0);          // bulletFlags
  // Character run 1: 6 characters, bold on + italic off.
  u32(body, 6);
  u32(body, 0x00000003); // CFMasks: bold | italic
  u16(body, 0x0001);     // CFStyle: bold set, italic clear
  // Character run 2: 6 characters, font 1, 32pt, explicit red.
  u32(body, 6);
  u32(body, 0x00070000); // CFMasks: typeface | size | color
  u16(body, 1);          // fontRef
  u16(body, 32);         // fontSize
