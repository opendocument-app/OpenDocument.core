...
template<typename T>
static void readPrimitive(std::istream &in, T &out) {
    in.read((char *) &out, sizeof(out));
}

static std::string readAsciiString(std::istream &in, const std::uint32_t length) {
    std::string result(length, ' ');
    in.read((char *) result.data(), result.size());
    return result;
}

static std::string readUtf16String(std::istream &in, const std::uint32_t length) {
...
static std::string readStringWithEncoding(std::istream &in, const TextEncoding encoding) {
...
struct TextAction {
    ...
    void read(std::istream &in, const VersionLength &vl, const TextEncoding encoding) {
        point.read(in);
...
static void writeColorStyle(std::ostream &out, const std::string &prefix, std::uint32_t color, bool set) {
    if (set) {
...
static void writeRectangle(std::ostream &out, const Rectangle &rect, SVMContext &context) {
    out << "<rect";
    out << " x=\"" << rect.left << "\"";
    out << " y=\"" << rect.top << "\"";
...
static void writePolygon(std::ostream &out, const std::string &tag, const std::vector<IntPair> &points,
        bool fill, SVMContext &context) {
    out << "<" << tag;
...
static void writeText(std::ostream &out, const IntPair &point, const std::string &text, SVMContext &context) {
    out << "<text";
    out << " x=\"" << point.x << "\"";
    out << " y=\"" << point.y << "\"";
...
static void translateAction(const ActionHeader &action, std::istream &in, std::ostream &out, SVMContext &context) {
    switch (action.type) {
...
bool Svm2Svg::translate(std::istream &in, std::ostream &out) {
    ...
    while (in.peek() != -1) {
        // TODO: check length fields should never exceed file size (limited inputstream?)
        ActionHeader action = {};
        action.read(in);
        ...
        if (left > 0) {
            ...
            LOG(ERROR) << -left << " bytes missing action "
                    << action.type << " version " << action.vl.version;
            return false;
        }
    }
