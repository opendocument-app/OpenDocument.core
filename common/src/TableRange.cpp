...
TableRange::TableRange(const TablePosition &from, const TablePosition &to) noexcept :
        from(from), to(to) {
}

TableRange::TableRange(const std::string &s) {
    const auto sep = s.find(':');
    if (sep == std::string::npos) throw std::invalid_argument("malformed table range " + s);
